...
Downloads terrain tiles from OpenTopoMap and packages them
...
Generates per-zoom MBTiles packs uploaded to Firebase Storage:
  {regionId}/terrain/terrain_z0-5.mbtiles
  {regionId}/terrain/terrain_z6.mbtiles
  {regionId}/terrain/terrain_z7.mbtiles
  ...
  {regionId}/terrain/terrain_z14.mbtiles
...
# Download settings (moderate concurrency for OpenTopoMap)
MAX_CONCURRENT = 8
...
# Zoom pack definitions (z0-5 as a range, then z6..z14 individually)
ZOOM_PACKS = {
    'z0-5':  {'minZoom': 0,  'maxZoom': 5,  'filename': 'terrain_z0-5.mbtiles'},
    'z6':    {'minZoom': 6,  'maxZoom': 6,  'filename': 'terrain_z6.mbtiles'},
    'z7':    {'minZoom': 7,  'maxZoom': 7,  'filename': 'terrain_z7.mbtiles'},
    'z8':    {'minZoom': 8,  'maxZoom': 8,  'filename': 'terrain_z8.mbtiles'},
    'z9':    {'minZoom': 9,  'maxZoom': 9,  'filename': 'terrain_z9.mbtiles'},
    'z10':   {'minZoom': 10, 'maxZoom': 10, 'filename': 'terrain_z10.mbtiles'},
    'z11':   {'minZoom': 11, 'maxZoom': 11, 'filename': 'terrain_z11.mbtiles'},
    'z12':   {'minZoom': 12, 'maxZoom': 12, 'filename': 'terrain_z12.mbtiles'},
    'z13':   {'minZoom': 13, 'maxZoom': 13, 'filename': 'terrain_z13.mbtiles'},
    'z14':   {'minZoom': 14, 'maxZoom': 14, 'filename': 'terrain_z14.mbtiles'},
}
...
def update_status(db, region_id, status):
    """Write terrain generation status to Firestore."""
    try:
        doc_ref = db.collection('districts').document(region_id)
        doc_ref.set({'terrainStatus': status}, merge=True)
    except Exception as e:
...
def generate_terrain():
    ...
    Generates per-zoom MBTiles packs with coastal buffer filtering,
    and uploads them to Firebase Storage.
    """
    ...
    try:
        update_status(db, region_id, {
            ...
            'message': f'Generating terrain tiles for {region["name"]} '
        ...
        for pack_name, pack_config in ZOOM_PACKS.items():
            ...
            mbtiles_name = f'{region_id} Terrain ({pack_name})'
            ...
            storage_path = f'{region_id}/terrain/{filename}'
            logger.info(f'  Uploading to {storage_path} ({size_mb:.1f} MB)...')

            update_status(db, region_id, {
                'state': 'uploading',
                'message': f'Uploading {pack_name} terrain pack ({size_mb:.0f} MB)...',
            })
        ...
        _combine_and_zip(bucket, region_id, 'terrain', region['bounds'], db, work_dir, logger)
        ...
        logger.info(f'=== Terrain generation complete for {region_id}: '
...
def package_terrain():
    ...
    try:
        _combine_and_zip(bucket, region_id, 'terrain', region['bounds'], db, None, logger)

        duration = time.time() - start_time
        logger.info(f'=== Terrain packaging complete for {region_id}: {duration:.1f}s ===')
...
def get_status():
    ...
    try:
        ...
        return jsonify({
            'regionId': region_id,
            'terrainStatus': data.get('terrainStatus'),
            'terrainData': data.get('terrainData'),
        })
    except Exception as e:
