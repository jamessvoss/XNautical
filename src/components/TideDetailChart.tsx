/**
 * TideDetailChart - Displays 36-hour tide prediction chart
 * Shows sinusoidally interpolated tide curve with current time marker
 */
...
function TideDetailChart({ 
  ...
  useEffect(() => {
    if (!visible) return;
    if (!tideStations || tideStations.length === 0) return;
    ...
    if (selectedStationId) {
      ...
      // Find station name
      const station = tideStations.find(s => s.id === selectedStationId);
      setStationName(station?.name || selectedStationId);
    } else if (currentLocation) {
      // Find nearest station
      const [lng, lat] = currentLocation;
      const nearest = findNearestTideStation(lat, lng, tideStations);
      if (nearest) {
  ...
  }, [visible, selectedStationId, currentLocation, tideStations]);
  ...
  const loadChartData = async () => {
    ...
    try {
      ...
      console.log(`[TIDE CHART] Loaded ${predictions.length} predictions for ${stationId}`);
      ...
      const points = generateTideChartCurve(events, chartStartTs, chartEndTs, 15);
      
      console.log(`[TIDE CHART] Generated ${points.length} curve points`);
      setChartPoints(points);
    } catch (err: any) {
      console.error('[TIDE CHART] Error loading data:', err);
  ...
  const { min: yMin, max: yMax } = useMemo(() => {
    return getChartRange(chartPoints);
  }, [chartPoints]);
  ...
  const touchedPoint = useMemo(() => {
    ...
    return { x: touchX, y, value, time: timeStr };
    ...
    <View style={[styles.container, { height: chartHeight }]}>
      ...
      {loading ? (
        ...
        <Svg key={`svg-${stationId}-${yMin.toFixed(1)}-${yMax.toFixed(1)}`} width={screenWidth} height={chartHeight - 25}>
          ...
          {/* Zero line - horizontal line at 0 ft */}
          {yMin < 0 && yMax > 0 && (() => {
            const zeroY = CHART_PADDING.top + chartInnerHeight - ((0 - yMin) / (yMax - yMin)) * chartInnerHeight - 25;
            return (
              <>
                <Line
                  x1={CHART_PADDING.left}
          ...
          {/* Tide curve - offset by -25 to match other elements */}
          ...
          {touchedPoint && (() => {
            ...
            const horizontalOffset = 80; // Offset from finger
            ...
            return (
              <>
                ...
                <SvgText
                  ...
                  fontWeight="bold"
                >
                  {touchedPoint.value.toFixed(1)} ft
                </SvgText>
                <SvgText
                  x={tooltipX}
                  y={tooltipY + 23}
...
export default memo(TideDetailChart);
